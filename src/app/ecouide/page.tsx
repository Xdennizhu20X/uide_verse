...
export default function EcoUidePage() {
  const [ecoProjects, setEcoProjects] = useState<Project[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const fetchEcoProjects = async () => {
      try {
        // Consulta para obtener solo proyectos ecológicos
        const q = query(
          collection(db, "projects"),
          where("isEcological", "==", true)
        )
        ...
        querySnapshot.forEach((doc) => {
          ...
          projects.push({
            ...
            comments: [],
            isEco: true, // Todos estos proyectos son ecológicos por definición
            likes: data.likes || 0,
          })
        })
        
        setEcoProjects(projects)
      } catch (error) {
        console.error("Error fetching eco projects:", error)
      } finally {
        setLoading(false)
      }
    }
            ...
            <Card className="h-full transition-transform duration-300 hover:scale-105 hover:shadow-lg">
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  +{new Set(ecoProjects.map(p => p.author)).size}
                </div>
          ...
          {ecoProjects.length > 0 ? (
            ...
            <Card>
              <CardContent className="p-6 text-center text-muted-foreground">
                No hay proyectos ecológicos para mostrar
              </CardContent>
            </Card>
          )}
        </div>
